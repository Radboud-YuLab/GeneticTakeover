function [dna_minV, dna_Ntot, dna_Nmol] = dnaGenome(kribo, kpol, intNflag, nucCount, roundToNearest)
    ...
    prob.Constraints.c3 = Nrnuc == C*kpol*x(3)*0.5;
    prob.Constraints.c4 = Naa == aaCount(1)*x(1) + aaCount(2)*( x(2)+x(3) ) + aaCount(3)*y(1);
    prob.Constraints.c5 = Ndnuc == 2*( nucCount(1)*x(7) + nucCount(2)*x(8) + nucCount(3)*( x(9)+x(10) ) + ...
    ...
    if exitflag1 > 0 && exitflag2 > 0
        ...
        dna_minV = double(minV) * 1e-12 / 1e-5; %times 1e-12 to convert to um3, divide by 1e-5 to manage output
        dna_Ntot = double(sum(res));
        if nucCount(4) == 0 && nucCount(5) == 0
            dna_Nmol = double(res(1:10));
        else
            dna_Nmol = double(res);
        end
